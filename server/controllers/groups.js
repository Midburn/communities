...
module.exports = class GroupsController {
  ...
  constructor () {
    this.DEFAULT_WHERE_OPTIONS = {record_status: constants.DB_RECORD_STATUS_TYPES.ACTIVE };
...
next (new GenericResponse (constants.RESPONSE_TYPES.JSON, {groups}));
  ...
  async addGroupMembers(req, res, next) {
      try {
          ...
          const currentMembers = await services.db.GroupMembers.findAll({where: { GroupId: req.params.groupId, ...this.DEFAULT_WHERE_OPTIONS}});
          const currentMembersIds = currentMembers.map(m => m.user_id.toString());
          const membersToAdd = req.body.member.filter(newMember => newMember.id && !currentMembersIds.includes(newMember.id.toString()));
    ...
    async removeGroupMembers(req, res, next) {
        try {
            ...
            const currentMembers = await services.db.GroupMembers.findAll({where: { GroupId: req.params.groupId, ...this.DEFAULT_WHERE_OPTIONS}});
            const currentMembersIds = currentMembers.map(m => m.user_id.toString());
            const membersToRemove = req.body.member.filter(newMember => newMember.id && currentMembersIds.includes(newMember.id.toString()));
