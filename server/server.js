...
class Server {
  ...
  initMiddlewares () {
    ...
    this.app.use (
      this.initRequesMetaKeysMiddleware (constants.REQUEST_METAKEYS)
      ...
      for (const key of keys) {
        if (req.headers[key]) {
          try {
            ...
            req.MetaKeys[key] = req.headers[key];
          }
        }
      }
      return next ();
      ...
      switch (genericResponse.type) {
        ...
        default:
          genericResponse = new GenericResponse (
            constants.RESPONSE_TYPES.ERROR,
            new Error (
              'Server Error - empty response sent to handler',
              genericResponse.stack
            )
          );
      }
