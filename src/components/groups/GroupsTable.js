...
class BaseGroupsTable extends React.Component {
  ...
  get CSVdata() {
    ...
    return groups.map(g => {
      const baseData = {
        [t(`${this.TRANSLATE_PREFIX}.table.groupName`)]: this.groupsService.getPropertyByLang(
          g,
          'name'
        ),
      ...
      const presaleData = presale
        ? {
            ...
            [t(`${this.TRANSLATE_PREFIX}.table.allocated`)]: this.getGroupTotalAllocated(
              g,
  ...
  getGroupQuota(group, key) {
    ...
    const groupQuota = groupQuotas[key || constants.UNPUBLISHED_ALLOCATION_KEY].find(
      adminAllocation => adminAllocation.group_id === group.id
    );
    if (!groupQuota) {
      return 0;
    }
  ...
  getTotalUnpublishedAllocations() {
    ...
    return groupQuotas[constants.UNPUBLISHED_ALLOCATION_KEY]
      ? groupQuotas[constants.UNPUBLISHED_ALLOCATION_KEY].reduce((result, value) => {
  ...
  getGroupTotalAllocated(group, allocationType) {
    ...
    return allocations.filter(
      allocation =>
        allocation.related_group === group.id && allocation.allocation_type === allocationType
    ).length;
  }
  ...
  extractNameFromJSON(manager) {
    try {
      ...
      return `${extraData.drupal_data.address.first_name} ${
        extraData.drupal_data.address.last_name
      }`;
    } catch (e) {
      return ' ';
    }
              ...
              return (
                <tr key={g.id}>
                  ...
                  <PermissableComponent permitted={presale && groupQuotas}>
                    ...
                    <td>{this.getGroupsBucketCount(g, constants.ALLOCATION_TYPES.PRE_SALE)}</td>
                    <td>{this.getGroupTotalAllocated(g, constants.ALLOCATION_TYPES.PRE_SALE)}</td>
                  </PermissableComponent>
                </tr>
              );
            })}
